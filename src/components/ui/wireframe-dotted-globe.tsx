...
export default function RotatingEarth({ width = 800, height = 600, className = "" }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const newScale = Math.max(radius * 0.5, Math.min(radius * 3, projection.scale() * (event.deltaY > 0 ? 0.9 : 1.1)))
  ...
  if (error) {
    return (
      <div className={`flex items-center justify-center p-8 ${className}`}>
        <p className="text-white/30 text-xs tracking-[0.2em]">GLOBE UNAVAILABLE</p>
      </div>
    )
  }
